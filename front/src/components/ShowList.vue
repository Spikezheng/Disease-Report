...
<el-card class="box-card">
    ...
    <el-table-column label="操作">
      <template slot-scope="forms">
        <el-button
          size="mini"
          @click="onSubmit(forms.row)"
          >提交上级</el-button>
        <el-button
          size="mini"
          @click="onSubmitCDC(forms.row)"
          >提交CDC</el-button>
        <el-button
          size="mini"
          type="danger"
...
<script>
    export default {
        ...
        methods: {
            //提交上级
            onSubmit(form) {
                this.$axios.post("/hospitalChartSubmit", {
                    id: form.id,
                    name: form.name,
                    age: form.age,
            ...
            //提交CDC
            onSubmitCDC(form) {
                this.$axios.post("/hospitalChartSubmitCDC", {
                    id: form.id,
                    name: form.name,
                    age: form.age,
                    idNumber: form.idNumber,
            ...
            onDel(form) {
                this.$axios.post("/hospitalChartDel", {
                    ...
                    if (successResponse.data.code === 200) {
                        ...
                        this.$axios.get("/getHospitalCharts"
                        ).then(successResponse => {
            ...
            //退出
            logout(){
              this.$options.methods.delCookie("hospital", this);
